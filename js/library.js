...
$(document).ready(function() {
	...
	$('#nav a[href*="#"]').click(function(){
		var tabsExists=window.location.hash.indexOf('#tabs-');
	//if (tabsExists>=0)alert(parseInt($(this).attr('href')));
	
		var hash = $(this).attr("href").split("#tabs-");
		var hash_pos = Number(hash[1].replace("#tabs-",""));
		var active_tab = (!isNaN(hash_pos) && hash_pos>0) ? hash_pos - 1 : 0;
	...
	$('a').click(function(){
		if (this.href.indexOf("http")==0 && this.href.indexOf(location.hostname)<0 && this.href.indexOf("fundsxpress.com")<0 && this.href.indexOf("okheritage.com")<0 ) this.href = "javascript:LinkAlert('" + this.href + "');";
	...
	var $s = $('.slideshow').slides({
		...
		nextText:	"<span class='fa fa-angle-right fa-3x'></span>",
		previousText:	"<span class='fa fa-angle-left fa-3x'></span>"
		});
	...
	$("a.control").click(function(e){
		...
		obj = $($(this).attr("href"));
		$(this).toggleClass("on");
		$( (obj.attr("id")=="login") ? "#navigation" : "#login" ).hide();
		obj.toggle();
		});
	...
	$("#weather #click").click(function(){
		...
		$("#click .fa").toggleClass("fa-angle-down fa-angle-up");
	...
	$("form[name=google]").addClass("form-inline");
	$("form[name=google] input[type=text]").addClass("form-control");
	$("form[name=google] input[type=submit]").addClass("btn btn-primary");
